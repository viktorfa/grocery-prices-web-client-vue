...
import { lunrSearch, loadIndex, loadObjects } from '@/search';
...
describe('lunrSearch', () => {
  ...
  test('should find items by prefix', async () => {
    const { ok, data: results } = await lunrSearch('medister');
    expect(ok).toBeTruthy();
    expect(
      results.find((x) => x.heading === 'Økologiske Medisterkaker')
    ).toBeDefined();
  });
  test('should find items by suffix', async () => {
    const { ok, data: results } = await lunrSearch('kaker');
    expect(ok).toBeTruthy();
    expect(
      results.find((x) => x.heading === 'Økologiske Medisterkaker')
    ).toBeDefined();
