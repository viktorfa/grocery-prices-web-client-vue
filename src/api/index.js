...
export const getGroceryOffer = async (uri) => {
  ...
  const response = await fetch(`${STRAPI_URL}/${strapiCollectionName}?uri=${uri}&_limit=1`)
  const {
    ok,
    data,
    error
  } = await getJsonFetchOption(response)
  if (ok && data.length > 0) {
    return {
      ok,
      data: data[0]
...
export const searchGroceryOffers = async (query) => {
  const url = 'https://75l1klfn37.execute-api.eu-central-1.amazonaws.com/dev/offers/search'
